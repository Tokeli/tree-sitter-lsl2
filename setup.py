...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_lsl", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_lsl": ["*.pyi", "py.typed"],
        "tree_sitter_lsl.queries": ["*.scm"],
    },
    ext_package="tree_sitter_lsl",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_lsl/binding.c",
                "src/parser.c",
            ],
            define_macros=[
